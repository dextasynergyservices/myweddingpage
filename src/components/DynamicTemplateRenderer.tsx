...
function replacePlaceholders(obj: unknown, userData: UserData): unknown {
  ...
  const replacementsObj = {
    "{brideName}":
      userData?.brideName || userData?.bride_name || userData?.bride || "Bride",
    "{bride_name}":
      userData?.brideName || userData?.bride_name || userData?.bride || "Bride",
    "{bride}":
      userData?.brideName || userData?.bride_name || userData?.bride || "Bride",
    "{groomName}":
      userData?.groomName || userData?.groom_name || userData?.groom || "Groom",
    "{groom_name}":
      userData?.groomName || userData?.groom_name || userData?.groom || "Groom",
    "{groom}":
      userData?.groomName || userData?.groom_name || userData?.groom || "Groom",
    ...
    "{date}":
      ...
      "Date",
    "{venue}":
      userData?.venue || userData?.location || userData?.place || "Venue",
    "{location}":
      userData?.venue || userData?.location || userData?.place || "Venue",
  } as Record<string, unknown>;
  const replacementEntries: [string, string][] = Object.entries(
    replacementsObj
  ).map(([k, v]) => [k, String(v ?? "")]);
...
const DynamicTemplateRendererComponent = ({
  ...
  const [localEditedSections, setLocalEditedSections] =
    useState<string[]>(editedSections);
  ...
  const processedSections = useMemo(() => {
    return (template?.sections ?? []).map((section) => {
      ...
      const sectionsObj = (userData?.sections ?? {}) as Record<
        string,
    ...
    <div className="space-y-8 relative">
      ...
      {sectionsToRender.map((section) => {
        ...
        // Check if it's an uppercase database layout (HERO, STORY, etc.)
          ...
          componentKey =
            `${templatePrefix}_${normalizedLayout}` as ComponentType;
        }
        // Use as-is (fallback)
        else {
          componentKey = section.layout as ComponentType;
        ...
        const RenderComponent = Component as React.ComponentType<
          Record<string, unknown>
        >;
        ...
        return (
          <div
            key={section.id}
            id={section.id}
            className="template-section relative"
          >
            {isEdited && editable && (
        ...
        <div className="bg-yellow-50 p-4 rounded-lg flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-yellow-700">
              Your {userPlan.name} plan includes {userPlan.maxComponents}{" "}
              sections. Upgrade to access all {processedSections.length}{" "}
...
export const DynamicTemplateRenderer = React.memo(
  DynamicTemplateRendererComponent
);
