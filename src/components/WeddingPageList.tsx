...
const prevPage = () => {
  ...
  return (
    <div className={`min-h-screen `}>
      {/* Hero */}
      <div className={`relative ${isDarkMode ? 'bg-indigo-950' : 'bg-indigo-900'} text-white py-20 rounded-4xl`}>
        <div className={`absolute inset-0 ${isDarkMode ? 'bg-black/40' : 'bg-black/30'} rounded-4xl`}></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="relative max-w-2xl mx-auto mb-8">
            ...
            <input
              ...
              className={`block w-full pl-10 pr-3 py-4 border ${
                isDarkMode ? 'border-gray-700 bg-gray-800 text-white placeholder-gray-400'
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[...Array(6)].map((_, i) => (
              <div key={i} className={`${cardBgClass} rounded-xl shadow-md overflow-hidden animate-pulse`}>
                ...
                <div className="p-6">
                  ...
                  <div className={`h-4 rounded w-2/3 mb-3 ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
                ...
                >
                  <Link href={`/weddings/${wedding.id}`} className="block">
                    ...
                    <div className="p-6">
                      ...
                      <div className={`flex items-center ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} mb-1`}>
                        <MapPin className="mr-2" />
                        <span>{wedding.location}</span>
                      </div>
                      <div className={`flex items-center ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} mb-3`}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {wedding.tags.map(tag => (
                          <span key={tag} className={`text-xs px-2 py-1 rounded-full ${tagBgClass}`}>
                            {tag}
                          </span>
                        ))}
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center mt-12">
                <nav className="flex items-center gap-1">
                  ...
                  >
                    <ChevronLeft className={`w-5 h-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`} />
                  </button>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full ${currentPage === totalPages ?
                      ...
                      'hover:bg-indigo-100 dark:hover:bg-indigo-900'}`}
                  >
                    <ChevronRight className={`w-5 h-5 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`} />
                  </button>
                </nav>
              </div>
            )}
