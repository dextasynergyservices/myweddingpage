...
const VendorPortal = () => {
  ...
  const categories = [
    ...
    { id: "Venue", label: "Venue", count: vendors.filter((v) => v.category === "Venue").length },
    {
      id: "Photography",
      label: "Photography",
      count: vendors.filter((v) => v.category === "Photography").length,
    },
    {
      id: "Florist",
      label: "Florist",
      count: vendors.filter((v) => v.category === "Florist").length,
    },
    {
      id: "Catering",
      label: "Catering",
  ...
  //   setVendors(
  //     vendors.map((vendor) => (vendor.id === id ? { ...vendor, status: newStatus } : vendor))
  //   );
  // };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            value: vendors.filter((v) => v.contractSigned).length,
            icon: FileText,
            color: "from-purple-500 to-pink-600",
          },
          {
            title: "Total Cost",
            value: `$${vendors.reduce((sum, v) => sum + v.price, 0).toLocaleString()}`,
            icon: DollarSign,
