...
export default function TemplatesAuditPage() {
  ...
  type VisibleColumns = { time: boolean; action: boolean; staging: boolean; details: boolean };
              ...
              return (
                <label key={key as string} className="text-xs flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) => setVisibleColumns((v) => ({ ...v, [key]: e.target.checked }))}
                  />{" "}
                  {key}
                </label>
              );
          ...
          <div className="ml-2 flex items-center gap-2">
            <button
              ...
              onClick={() => {
                ...
                searchToastRef.current = toasts.toast({ title: "Searching..." });
                load();
                // persist to URL
                try {
                  const params = new URLSearchParams();
